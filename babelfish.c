...
volatile u32 syscall_lr = 0;
u32 syscall_handler_orig = 0;

extern void syscall_shim(void);
...
static u32 xchange_osvers_and_patch(u32 version)
{
	...
	dprintf("xchange_osvers_and_patch(%x)\n", version);

	if (version && (version != oldvers))
	{
//		dprintf("! Set OS version to %u !\n", version);
		write32(0x3140, version);
//		dc_flush();
	}
	...
	dprintf("! Returning old OS version %x !\n", oldvers);
...
static void stuff_EXI_stub_wrapper(u32 which_stub, u32 *insns, u32 len)
{
	...
	dprintf("stuff_EXI_stub(%x, %x, %x)\n", which_stub, (u32)insns, len);
	dprintf("Looking for fwrite\n");
	dprintf("81330000: %08x %08x %08x %08x\n", mem1[0], mem1[1], mem1[2], mem1[3]);
	...
	for (addr = 0; addr < 0x1800000; addr += 4)
	{
		if (!memcmp((void *)addr, sig_fwrite, sizeof(sig_fwrite)))
		{
			...
			dprintf("found fwrite at %x, patching\n", addr);
...
static void find_powerpc_reset(void)
{
	...
	for (i = 0; i < SEARCH_RANGE; i++)
	{
		if ((kernel[i] == magic[0]) && (kernel[i + 1] == magic[1]))
		{
			dprintf("Found powerpc_reset magic at %x\n", 0xFFFF0000 + (i * 4));
			break;
		}
	}
	...
	for (; i > 0; i--)
	{
		...
		{
			dprintf("Found powerpc_reset start at %x\n", 0xFFFF0000 + (i * 4));
			break;
		}
	}
	...
	if (i == 0x10000)
	{
		dprintf("Couldn't find powerpc_reset start\n");
		return;
	}
	...
	for (i = 0; i < SEARCH_RANGE; i++)
	{
		...
		{
			dprintf("Found powerpc_reset reference at %x\n", 0xFFFF0000 + (i * 4));
			break;
		}
	}
	...
	if (i == 0x10000)
	{
		dprintf("Couldn't find powerpc_reset reference\n");
		return;
	}

//	kernel[i] = (u32)&powerpc_reset_wrapper;
	dprintf("Replaced powerpc_reset reference with %x\n", kernel[i]);
}
#endif
...
static u32 *find_xchange_osvers(void)
{
	...
	for (i = 0; i < SEARCH_RANGE; i++)
	{
		if (kernel[i] == magic[0])
		{
			dprintf("Found xchange_osvers start at %x\n", 0xFFFF0000 + (i * 4));
...
static void do_kernel_patches(u32 size)
{
...
#if 0
	dprintf("starlet_syscall_handler vector: %x\n", kernel_mem32[9]);
	dprintf("svc_handler vector2: %x\n", kernel_mem32[10]);
#endif	
	 ...
	 */
	if (kernel_mem32[9] != (u32)syscall_shim)
	{
		syscall_handler_orig = kernel_mem32[9];
		kernel_mem32[9] = (u32)syscall_shim;
	}
...
#else
	printf("%s(0x%08x, 0x%x, 0x%08x, stacksize: 0x%x, prio: 0x%x, attr: 0x%x)\n", __FUNCTION__, (u32)entry, (u32)arg, (u32)stack, stacksize, prio, attr);
#endif
	return retval;
}
...
#else
	printf("%s(id: 0x%08x, val: 0x%08x)\n", __FUNCTION__, id, (u32)arg);
#endif
	return retval;
}
...
#else
	printf("%s(id: %d, prio: %d)\n", __FUNCTION__, id, prio);
#endif
	return retval;
}
...
#else
	printf("%s(0x%08x, cnt: %d)\n", __FUNCTION__, (u32)msg, size);
#endif
	return retval;
}
...
#else
	printf("%s(0x%08x, %08x, flag: %08x)\n", __FUNCTION__, id, msg, flag);
#endif
	return retval;
}
...
#else
	printf("%s(%d, %d, %d, 0x%08x)\n", __FUNCTION__, val, interval, mqid, msg);
#endif
	return retval;
}
...
#else
	printf("%s(%d, 0x%08x, 0x%x)\n", __FUNCTION__, fd, (u32)buf, count);
#endif
	return retval;
}
#endif
...
#else
	printf("%s(%d, 0x%x, 0x%08x, 0x%x, %08x, 0x%x)\n", __FUNCTION__, fd, cmd, (u32)in, inlen, (u32)out, outlen);
#endif
	return retval;
}
...
#else
	printf("%s(%d, 0x%x, %d, %d, 0x%08x)\n", __FUNCTION__, fd, cmd, read, written, (u32)vec);
#endif
	return retval;
}
...
#else
	printf("%s(%s, %d, %08x, %08x)\n", __FUNCTION__, filename, mode, id, (u32)reply);
#endif
	return retval;
}
...
#else
	printf("%s(%d, 0x%08x, 0x%x, %08x, %08x)\n", __FUNCTION__, fd, (u32)buf, count, id, (u32)reply);
#endif
	return retval;
}
...
#else
	printf("%s(%d, %d, %d, %08x, %08x)\n", __FUNCTION__, fd, offset, whence, id, (u32)reply);
#endif
	return retval;
}
...
#else
	printf("%s(%d, 0x%x, 0x%08x, 0x%x, %08x, 0x%x, %08x, %08x)\n", __FUNCTION__, fd, cmd, (u32)in, inlen, (u32)out, outlen, id, (u32)reply);
#endif
	return retval;
}
...
#else
	printf("%s(%d, 0x%x, %d, %d, 0x%08x, %08x, %08x)\n", __FUNCTION__, fd, cmd, read, written, (u32)vec, id, (u32)reply);
#endif
	return retval;
}
...
#else
	printf("%s(pool: 0x%08x, size: %d, debug: %d)\n", __FUNCTION__, pool, size, dbg);
#endif
	return retval;
}
...
#else
	printf("%s(buf: 0x%08x, len: %d)\n", __FUNCTION__, (u32)iob, len);
#endif
	return retval;
}
...
#else
	printf("%s(%s, reset: %d, version: %d)\n", __FUNCTION__, filename, r1, filesize);
#endif
	return retval;
}
...
#else
	printf("%s(offset: 0x%08x, value: %d)\n", __FUNCTION__, offset, value);
#endif
	return retval;
}
#endif
...
static void handle_syscall_table(u32 *syscall_table, u32 size)
{
	...
	num_syscalls = i;
	dprintf("Syscall table @ 0x%x: %x entries\n", (u32)syscall_table, num_syscalls);
#else
	(void)size;
#endif
	/*cookie =*/ irq_kill();
...
static void *find_stuff_EXI_stub(void)
{
	...
	for (i = 0; i < SEARCH_RANGE; i++)
	{
		...
		{
			dprintf("Found ppc_stub1 reference at %x\n",0xFFFF0000 + (i * 4));
			break;
		}
	}
	...
	for (; i > 0; i--)
	{
		...
		{
			dprintf("Found stuff_EXI_stub start at %x\n", 0xFFFF0000 + (i * 4));
			break;
		}
	}
	...
	if (i == 0x10000)
	{
		dprintf("Couldn't find stuff_EXI_stub start\n");
		return NULL;
	}
	...
	for (i = 0; i < SEARCH_RANGE; i++)
	{
		...
		{
			dprintf("Found stuff_EXI_stub_addr reference at %x\n", 0xFFFF0000 + (i * 4));
			break;
		}
	}
	...
	if (i == 0x10000)
	{
		dprintf("Couldn't find stuff_EXI_stub reference\n");
		return NULL;
	}

	kernel[i] = (u32)&stuff_EXI_stub_wrapper;
	dprintf("Replaced stuff_EXI_stub reference with %x\n", kernel[i]);
	return NULL;
}
#endif
