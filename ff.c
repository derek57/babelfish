...
//	WORD ic;
	...
	if (clst == 0) {	/* Static table */
		...
		WORD ic = SS(dj->fs) / 32 * dj->fs->csize;	/* Indexes per cluster */
	...
	if (!(i % (SS(dj->fs) / 32))) {	/* Sector changed? */
		...
		if (dj->sclust == 0) {	/* Static table */
			...
			if (((i / (SS(dj->fs) / 32)) & (dj->fs->csize - 1)) == 0) {	/* Cluster changed? */
				DWORD clst = get_cluster(dj->fs, dj->clust);			/* Get next cluster */
...
)
{
	...
	if (ofs > 0) {
		DWORD clst;
		DWORD bcs = (DWORD)fp->fs->csize * SS(fp->fs);	/* Cluster size (byte) */
		if (ifptr > 0 &&
