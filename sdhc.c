...
#define HREAD1(hp, reg)		(bus_space_read_1((hp)->ioh, (reg)))
#define HREAD2(hp, reg)		(bus_space_read_2((hp)->ioh, (reg)))
#define HREAD4(hp, reg)		(bus_space_read_4((hp)->ioh, (reg)))
#define HWRITE1(hp, reg, val)	bus_space_write_1((hp)->ioh, (reg), (val))
#define HWRITE2(hp, reg, val)	bus_space_write_2((hp)->ioh, (reg), (val))
#define HWRITE4(hp, reg, val)	bus_space_write_4((hp)->ioh, (reg), (val))
#define HCLR1(hp, reg, bits)	HWRITE1((hp), (reg), HREAD1((hp), (reg)) & ~(bits))
#define HCLR2(hp, reg, bits)	HWRITE2((hp), (reg), HREAD2((hp), (reg)) & ~(bits))
#define HSET1(hp, reg, bits)	HWRITE1((hp), (reg), HREAD1((hp), (reg)) | (bits))
#define HSET2(hp, reg, bits)	HWRITE2((hp), (reg), HREAD2((hp), (reg)) | (bits))


static struct sdhc_host sc_host;
